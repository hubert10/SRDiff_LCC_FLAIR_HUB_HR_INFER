...
from tasks.module_setup import get_input_img_sizes, build_segmentation_module


def training_stage(config: Dict, data_module, out_dir: Path) -> OrderedDict:
    """
    ...
    best_trained_state_dict = torch.load(
        ckpt_callback.best_model_path, map_location=torch.device("cpu")
    )["state_dict"]
...
def predict_stage(
    ...
    if config["tasks"].get("metrics_only", False) and not config["tasks"].get(
        "predict", False
    ):
