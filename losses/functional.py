...
def label_smoothed_nll_loss(
    ...
    if ignore_index is not None:
        ...
        nll_loss = -lprobs.gather(dim=dim, index=target.type(torch.int64))
...
def label_smoothed_weighted_nll_loss(
    lprobs: torch.Tensor,
    target: torch.Tensor,
    epsilon: float,
    ...
    if weight is not None:
        # Flatten → lookup weights → reshape back
        w = weight.to(nll_loss.device).take(target.squeeze(dim).view(-1))
        w = w.view_as(target.squeeze(dim))
        nll_loss = nll_loss * w
