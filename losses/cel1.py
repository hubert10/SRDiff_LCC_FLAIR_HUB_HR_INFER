...
def get_region_proportion(
    x: torch.Tensor, valid_mask: torch.Tensor = None
) -> torch.Tensor:
...
class CompoundLoss(nn.Module):
    ...
    def get_gt_proportion(
        self, mode: str, labels: torch.Tensor, target_shape, ignore_index: int = 255
    ):
    ...
    def get_pred_proportion(
        self, mode: str, logits: torch.Tensor, temp: float = 1.0, valid_mask=None
    ):
        if mode == MULTICLASS_MODE:
